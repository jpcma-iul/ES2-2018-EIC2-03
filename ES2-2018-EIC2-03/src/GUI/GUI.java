...
public class GUI {
	...
	/*This is the functional part of the GUI, where all the listeners are made to react to user input*/
	private void initializeActionListeners(){
		...
		/*Add Listener to Problem Validation Button with explanation of how the problem name should be written*/
		...
		IdealDeadline.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(ideal>maximum) MaximumDeadline.setSelectedIndex(ideal-1);
			}
		});
		MaximumDeadline.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				int ideal = Integer.valueOf(String.valueOf(IdealDeadline.getSelectedItem()).split("\\s")[0]);
				int maximum = Integer.valueOf(String.valueOf(MaximumDeadline.getSelectedItem()).split("\\s")[0]);
				if(ideal>maximum) IdealDeadline.setSelectedIndex(maximum-1);
			}
		});
		/*Add Listener to the variable quantity JComboBox, which will then trigger the whole variable naming and configurating
		 * as well as add a JComboBox, a JSlider and a JLabel respectively*/
				...
				if(chosen==decision.OK_OPTION) {
					...
					for(int i=0;i<howMany;i++) {
						...
						while(isRepeatedVariableName(inputVariableName) || !(isValidVariableName(inputVariableName))) {
							decision.showMessageDialog(frame, "The name you typed was already given to a different variable or is invalid. \nTry again as this is not allowed!", "ATTENTION", JOptionPane.ERROR_MESSAGE);
					...
					int count = 0;
					/*Create a String[] with all the Variable names in order to create the JComboBox*/
					for(Variable v:variableTable.getVariables()) {
						decisionVariables[count]=v.getVarName();
						count++;
					}
					variablesName = new JComboBox(decisionVariables);
					/*With the JComboBox created, we add to the framework the ComboBox of the variables, the JSlider and the JLabel containing the
					 * value on the JSlider. Also allows the user now to change the Variable Range from default to one of his choice.*/
		...
		/*Represent the JSlider's immediate value in a JLabel. Allow value ranges between -99.9 and 99.9 maximum for double
		 * and between -999 and 999 for integer and binary*/
	...
	/*Throws window with 2 JComboBoxes to define both the minimum and maximum value that specific variable can have*/
		...
		case REAL:
			...
			if(String.valueOf(variablesWeight.getValue()).substring(0,1).equals("-")) {
				...
				if(String.valueOf(variablesWeight.getValue()).length()==4) {
					weightValue.setText(String.valueOf(variablesWeight.getValue()).substring(0, 3)+"."+String.valueOf(variablesWeight.getValue()).substring(3,4));
				}
	...
	private boolean isRepeatedVariableName(String nameToTest){
		int count = 0;
		for(Variable v:variableTable.getVariables()) {
			if(v.getVarName().equals(nameToTest)) count++;
		}
